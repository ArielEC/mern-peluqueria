...
export function AdminPageHeader({ title, subtitle, actionLabel, actionIcon = 'add', onAction, searchValue, onSearch, searchPlaceholder = 'Buscar...' }) {
  return (
    <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
      <div>
        <h2 className="text-[2rem] font-extrabold tracking-tight text-[#131b2e] leading-tight">{title}</h2>
        ...
        {onSearch !== undefined && (
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2.5 bg-[#eaedff] border-0 rounded-xl text-[0.875rem] text-[#131b2e] w-64 outline-none focus:ring-2 focus:ring-[#6b38d4] placeholder:text-[#494454]/50"
            />
          </div>
        )}
        {onAction && (
          <button
            onClick={onAction}
            className="flex items-center gap-2 bg-[#6b38d4] text-white px-5 py-2.5 rounded-xl font-bold text-[0.875rem] hover:brightness-110 active:scale-95 transition-all shadow-sm shadow-[#6b38d4]/20"
          >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#f2f3ff]/60 border-b border-[#cbc3d7]/15">
                {columns.map((col) => (
                  <th
                    key={col.key}
                    className={`px-6 py-4 text-[0.7rem] font-bold uppercase tracking-wider text-[#494454] ${col.className ?? ''}`}
                  >
                    {col.label}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-[#cbc3d7]/10">
              {loading ? (
                ...
                <tr>
                  <td colSpan={columns.length} className="px-6 py-16 text-center text-[#494454]">
...
export function AdminModal({ open, onClose, title, subtitle, accentColor = '#6b38d4', children, footer }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-xl shadow-2xl z-10 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        {footer && <div className="p-6 pt-0 shrink-0 flex gap-3">{footer}</div>}
      </div>
    </div>
  );
}
...
export const inputCls = "w-full bg-[#f2f3ff] border-0 rounded-lg px-3 py-2.5 text-[0.875rem] text-[#131b2e] placeholder:text-[#494454]/50 outline-none focus:ring-2 focus:ring-[#6b38d4]";
